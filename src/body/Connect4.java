...
public class Connect4 extends Frame implements BoardWindow {
	...
	void init(int PLAYING, int difficulty) { // 보드를 초기화
		...
		for (int i = 0; i < ROWS; i++) {
			for (int j = 0; j < COLS; j++) {
				...
				case 1:
					...
					g.fillOval(BOARD_X + j * BOARD_W_INTERVAL, BOARD_Y + BOARD_H - (i + 1) * BOARD_H_INTERVAL,
							BOARD_W_INTERVAL, BOARD_H_INTERVAL);
					g.setColor(Color.black);
					g.drawOval(BOARD_X + j * BOARD_W_INTERVAL, BOARD_Y + BOARD_H - (i + 1) * BOARD_H_INTERVAL,
							BOARD_W_INTERVAL, BOARD_H_INTERVAL);
	...
	private void gameover() { // 게임이 끝났는지 체크해준다.
		...
		case 1:
			if (MYPLAYING == 1)
				JOptionPane.showMessageDialog(this, "Player 1(CPU) WIN!");
			else
		...
		case 2:
			if (MYPLAYING == 2)
				JOptionPane.showMessageDialog(this, "Player 2(CPU) WIN!");
			else
	...
	public void setDifficulty(int dif) {
		...
		case LOW: // low
			...
			if (menuDifficultyLow.getState() == false)
				menuDifficultyLow.setState(true);
		...
		case MODERATE: // moderate
			...
			if (menuDifficultyModerate.getState() == false)
				menuDifficultyModerate.setState(true);
			menuDifficultyHigh.setState(false);
		...
		case HIGH: // high
			...
			if (menuDifficultyHigh.getState() == false)
				menuDifficultyHigh.setState(true);
			menuDifficultyModerate.setState(false);
			menuDifficultyLow.setState(false);
			DIFFICULTY = HIGH;
