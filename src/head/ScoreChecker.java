...
public class ScoreChecker {
		...
		for (int row = 5; row >= 3; row--) {
			for (int col = 0; col < 7; col++) {
				if (board[row][col] == 1 && board[row - 1][col] == 1 && board[row - 2][col] == 1
						&& board[row - 3][col] == 1) {
					return 1;
				} else if (board[row][col] == 2 && board[row - 1][col] == 2 && board[row - 2][col] == 2
						&& board[row - 3][col] == 2) {
					return 2;
				}
			}
		...
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col <= 3; col++) {
				if (board[row][col] == 1 && board[row][col + 1] == 1 && board[row][col + 2] == 1
						&& board[row][col + 3] == 1) {
					return 1;
				} else if (board[row][col] == 2 && board[row][col + 1] == 2 && board[row][col + 2] == 2
						&& board[row][col + 3] == 2) {
					return 2;
				}
			}
		...
		for (int row = 0; row <= 2; row++) {
			for (int col = 0; col < 4; col++) {
				if (board[row][col] == 1 && board[row + 1][col + 1] == 1 && board[row + 2][col + 2] == 1
						&& board[row + 3][col + 3] == 1) {
					return 1;
				} else if (board[row][col] == 2 && board[row + 1][col + 1] == 2 && board[row + 2][col + 2] == 2
						&& board[row + 3][col + 3] == 2) {
					return 2;
				}
			}
	...
	/*
	 * 한 개의 돌이 한 줄에 놓여있는 지 검사
	 * 
	 * @input: player = 누구의 입장에서 계산하는지
	 * 
	 * @input: board = 평가할 현재 board의 상태를 의미
	 */
