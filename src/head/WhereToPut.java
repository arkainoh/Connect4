...
public class WhereToPut implements BoardWindow{
	...
	public static int evaluate(int player, int[][] board, int stoneNum) {
		 ...
		 * @input: board에 놓인 돌의 상태를 argument로 받고 이것을 root node로 설정한다.
		 * 그리고 minmax 알고리즘을 수행 
		...
		//GameTree gt = new GameTree(player, board, stoneNum, depth); //DEPTH만큼의 수 앞을 내다보는 트리 생성
		//Minimax(gt);
		//ABPruningMinimax(gt);
	...
	private static int newabpruningMinimax(StateNode parentNode, int player, int a, int b, int depth) {
		...
		if(depth == 0){ //depth가 0이 되었다면 자기 자신의 점수를 내고 해당 점수를 위로 올림
			...
			if(parentNode.getPlayer() == player) { //만약 해당 노드가 자신이 두어야 할 선택지라면, 
				//자식 노드들의 추정값 중 가장 작은 값을 취한다. (자식 노드의 level은 상대가 두어야 할 선택지이기 때문)
				for(StateNode child : parentNode.getChildren()) {
					score = newabpruningMinimax(child, player, a, b, depth-1);
					if(b>score) b = score;
					if(b<=a) break; //a값보다 b값이 커야 더 위의 조상 중 maximizer가 값을 취할 수 있다. 그렇지 않을 경우 버려도 상관없다.
				}
				parentNode.setScore(b);
				return b;
				
			} else { //만약 자신이 두어야 할 선택지가 아니라면,
				//자식들 중 가장 큰 값을 취한다.
				for(StateNode child : parentNode.getChildren()) {
					score = newabpruningMinimax(child, player, a, b, depth-1);
					if(a<score) a = score;
					if(b<=a) break; //b보다 a가 작은 값이 나와야 하는데, a가 한 번 큰 순간 절대 b보다 작아질 수 없다.
				}
	...
	private static void Minimax(GameTree gt) { //alpha-beta pruning이 없는 단순 Minimax
		...
		} //자식 노드가 하나도 없을 경우를 대비해서 트리의 마지막 층을 구하는 단계이다. 마지막 층부터 검사해서 아무 노드도 없으면 마지막 층을 한단계 위로 설정한다.

		/*
		if(lastLevel == 0) {
		//만약 마지막 레벨이 rootNode라면 더이상 둘 수 없다는 의미이므로 이미 getWinner()에서 draw로 끝이날 것이므로 이 부분은 생략해도 된다. 
			return 마지막 남은 column
		}
		 */
		...
		for (int i = lastLevel - 1; i > 0; i--) {
			...
			if (!curLevel.isEmpty()) {
				...
				if (curLevel.get(0).getPlayer() == player) { //만약 해당 level이 자신이 두어야 할 선택지라면,
					for (StateNode node : curLevel) {
						if(node.getChildrenNum()!=0)
							minNode(node); //자식 노드들의 추정값 중 가장 작은 값을 취한다. (자식 노드의 level은 상대가 두어야 할 선택지이기 때문)
						else
							node.setScore(ScoreChecker.getScore(player, node.getBoard())); //자식이 하나도 없으면 자기 자신의 스코어를 계산한다.
					}
				} else {
					for (StateNode node : curLevel) {
						if(node.getChildrenNum()!=0)
	...
	private static int abpruningMinimax(StateNode parentNode, int player, int a, int b) {
		...
		if(parentNode.getChildrenNum() == 0){ //자식이 하나도 없다면 자기 자신의 점수를 내고 해당 점수를 위로 올림
			score = ScoreChecker.getScore(player, parentNode.getBoard());
			parentNode.setScore(score);
			return score;
		}
	...
	public static int[][] putInCol(int[][] board, int player, int col) {
		if (player != 1 && player != 2)
			return null;
		//col의 범위가 0~6이 아닌 경우 어차피 index outofbound exception이 발생할 것임
		...
		return null; //만약 꽉 차있는 column에 put했을 시 null을 반환 -> alpha-beta pruning을 위해!
	}
	...
	private static void maxNode(StateNode node) {
		...
		for(StateNode child : node.getChildren()) {
			if(maxScore<=child.getScore()) maxScore = child.getScore();
		}
	...
	private static void minNode(StateNode node) {
		...
		for(StateNode child : node.getChildren()) {
			if(minScore>=child.getScore()) minScore = child.getScore();
		}
